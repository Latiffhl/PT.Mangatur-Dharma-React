...
import { getListEvents } from '@/lib/event.api'; // Impor fungsi API
import { AppConfig } from '@/lib/config'; // Impor AppConfig
import { cn } from '@/lib/utils';

// Komponen kartu berita yang dapat digunakan kembali
    ...
    >
      ...
      <div className="p-4 flex-1">
        <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
          {/* Anda mungkin perlu menambahkan data Author dan Date dari API jika ada */}
...
const Aktifitas = () => {
  ...
  const fetchNews = async () => {
    setLoading(true);
    // Asumsi ApiDB berada di AppConfig. Jika tidak, Anda perlu menentukannya di sini
    const data = await getListEvents(1, 10, category);
    setNewsData(data.list);
    setLoading(false);
  };

  // Efek untuk memanggil API saat kategori berubah
  useEffect(() => {
    fetchNews();
  }, [category]);

  // Pisahkan data untuk Top News dan Latest News
