...
import { getListAwards } from '@/lib/awards.api'; // Impor fungsi API
...
const Penghargaan = () => {
  ...
  // Fungsi untuk merender kartu penghargaan dari data API
  const renderAwardCard = (award, index) => {
    const imageUrl = `http://172.25.0.114:8080/a7mangatur/${award.thumbnail}`; // Gunakan URL gambar dari data
    return (
      <div key={index} className="bg-white p-6 rounded-lg shadow-lg border border-gray-300 transition-all duration-300 hover:shadow-xl hover:-translate-y-2">
        ...
        <h3 className="font-bold text-2xl text-center">{award.Judul}</h3>
        <p className="text-center mt-2">{award.Deskripsi}</p>
      </div>
    );
  };
